	...
	class YITH_WooCommerce_Gift_Cards {
		...
		public function init_hooks() {
			...
			if ( defined( 'ELEMENTOR_VERSION' ) ) {
				require_once YITH_YWGC_DIR . 'includes/third-party/elementor/class-ywgc-elementor.php';
			}
		...
		public function on_plugin_init() {
			...
			if ( apply_filters( 'ywgc_update_database_bool', false ) ){
		...
		public function register_custom_post_statuses() {
			...
			register_post_status(
				...
				array(
					...
					/* translators: %s: html */
					'label_count'               => _n_noop( esc_html__( 'Disabled', 'yith-woocommerce-gift-cards' ) . '<span class="count"> (%s)</span>', esc_html__( 'Disabled', 'yith-woocommerce-gift-cards' ) . ' <span class="count"> (%s)</span>' ),//phpcs:ignore --_n scaping
				)
			);
			...
			register_post_status(
				...
				array(
					...
					'post_type'                 => array( 'gift_card' ),
					/* translators: %s: html */
					'label_count'               => _n_noop( esc_html__( 'Dismissed', 'yith-woocommerce-gift-cards' ) . '<span class="count"> (%s)</span>', esc_html__( 'Dismissed', 'yith-woocommerce-gift-cards' ) . ' <span class="count"> (%s)</span>' ),//phpcs:ignore --_n scaping
				)
			);
			...
			register_post_status(
				...
				array(
					...
					'post_type'                 => array( 'gift_card' ),
					/* translators: %s: html */
					'label_count'               => _n_noop( esc_html__( 'Code not valid', 'yith-woocommerce-gift-cards' ) . '<span class="count"> (%s)</span>', esc_html__( 'Code not valid', 'yith-woocommerce-gift-cards' ) . ' <span class="count"> (%s)</span>' ),//phpcs:ignore --_n scaping
				)
			);
		}
		 ...
		 *
		 * @param YITH_YWGC_Gift_Card|WC_Product $obj obj.
		 *
		 ...
		 *
		 * @param YWGC_Gift_Card $gift gift.
		...
		public function check_gift_card( $gift, $remove = false ) {
			...
			if ( ! $remove ) {
				...
				if ( WC()->cart->total < $ywgc_minimal_car_total ) {
					$notice_string = esc_html__( 'In order to use the gift card, the minimum total amount in the cart has to be ', 'yith-woocommerce-gift-cards' ) . $ywgc_minimal_car_total . get_woocommerce_currency_symbol();
					wc_add_notice( ( $notice_string ), 'error' );
					return false;
				}
			}
