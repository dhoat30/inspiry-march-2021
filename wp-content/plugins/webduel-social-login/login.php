...
function jwtTokenGoogleLogin($username, $password)
{
    ...
    $url = '';
    if (strstr($_SERVER['SERVER_NAME'], 'localhost')) {
        $url = "https://localhost/wp-json/jwt-auth/v1/token";
    } else {
        $url = "https://inspiry.co.nz/wp-json/jwt-auth/v1/token";
    }
    curl_setopt_array($curl, array(
        CURLOPT_URL => "https://localhost/wp-json/jwt-auth/v1/token",
    ...
    // sett auth cookie 
    setcookie("inpiryAuthToken", $obj->user_email, time() + (86400 * 30), "/"); // 86400 = 1 day
}
