<?php
namespace TrsVendors\Dgm\Arrays;
...
class Arrays
{
    ...
    public static function map($list, $callback)
    {
        ...
        foreach ($list as $key => $item) {
            $result[$key] = call_user_func((\TrsVendors_CCR::kallable($callback)), $item);
        }
    ...
    public static function reduce($input, $callback, $carry = null)
    {
        foreach ($input as $item) {
            $carry = call_user_func((\TrsVendors_CCR::kallable($callback)), $carry, $item);
        }
    ...
    public static function filter($input, $callback = null)
    {
        ...
        foreach ($input as $key => $item) {
            if (call_user_func((\TrsVendors_CCR::kallable($callback)), $item)) {
                $result[$key] = $item;
            } else {
                unset($result[$key]);
            }
