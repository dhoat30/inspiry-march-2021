...
class PackageConverter
{
     ...
     * @return array
     */
    public static function fromCoreToWoocommerce(\TrsVendors\Dgm\Shengine\Interfaces\IPackage $package)
    {
        $wcpkg = array();
    ...
    public static function fromWoocommerceToCore2(
        ...
        if ($includeVirtualItemsIfPackageIsRoot && self::isGlobalPackage($_package, $cart)) {
            ...
            $deferred = new \TrsVendors\Deferred\Deferred(function() use($fltr, $fltrcb, $fltpr) {
        ...
        foreach ((array)@$_package['contents'] as $_item) {
            ...
            $price = new \TrsVendors\Dgm\Shengine\Model\Price(
            ...
            while ($quantity--) {
                $item = new \TrsVendors\Dgm\Shengine\Woocommerce\Model\Item\WpmlAwareItem(
        ...
        if (($dest = @$_package['destination']) && @$dest['country']) {
            ...
            $destination = new \TrsVendors\Dgm\Shengine\Model\Destination(
                $dest['country'],
                @$dest['state'],
                @$dest['postcode'],
                @$dest['city'],
                new \TrsVendors\Dgm\Shengine\Model\Address(@$dest['address'], @$dest['address_2'])
            );
        }

        $customer = null;
        if (isset($_package['user']['ID'])) {
            $customer = new \TrsVendors\Dgm\Shengine\Model\Customer($_package['user']['ID']);
        }
        ...
        if ($preferCustomPackagePriceOverPerItemPrices && self::isGlobalPackage($_package, $cart)) {
            $customPackagePrice = new \TrsVendors\Dgm\Shengine\Model\Price(
        ...
        return new \TrsVendors\Dgm\Shengine\Model\Package($items, $destination, $customer, $coupons, $customPackagePrice);
    }
    ...
    private static function makeWcItems(\TrsVendors\Dgm\Shengine\Interfaces\IPackage $package)
    {
        $wcItems = array();

        $lineGrouping = new \TrsVendors\Dgm\Shengine\Grouping\AttributeGrouping(new \TrsVendors\Dgm\Shengine\Attributes\ProductVariationAttribute());
        ...
        foreach ($lines as $line) {
            ...
            $product = null; {
                ...
                if ($item instanceof \TrsVendors\Dgm\Shengine\Woocommerce\Model\Item\WoocommerceItem) {
            ...
            $wcItem = array(); {
                ...
                $wcItem['line_total'] = $line->getPrice(\TrsVendors\Dgm\Shengine\Model\Price::WITH_DISCOUNT);
                $wcItem['line_tax'] = $line->getPrice(\TrsVendors\Dgm\Shengine\Model\Price::WITH_DISCOUNT | \TrsVendors\Dgm\Shengine\Model\Price::WITH_TAX) - $wcItem['line_total'];
                $wcItem['line_subtotal'] = $line->getPrice(\TrsVendors\Dgm\Shengine\Model\Price::BASE);
                $wcItem['line_subtotal_tax'] = $line->getPrice(\TrsVendors\Dgm\Shengine\Model\Price::WITH_TAX) - $wcItem['line_subtotal'];
            }
    ...
    private static function getCustomerId(\TrsVendors\Dgm\Shengine\Interfaces\IPackage $package)
    {
    ...
    private static function getDestination(\TrsVendors\Dgm\Shengine\Interfaces\IPackage $package)
    {
    ...
    {
        return new \TrsVendors\Dgm\Shengine\Model\Dimensions(
    ...
    private static function getProductAttr(WC_Product $product, $attr)
    {
        if (version_compare(WC()->version, '3.0', '>=')) {
            switch ((string)$attr) {
                ...
                default:
                    return call_user_func(array((\TrsVendors_CCR::klass($product)), "get_{$attr}"));
            }
        }
