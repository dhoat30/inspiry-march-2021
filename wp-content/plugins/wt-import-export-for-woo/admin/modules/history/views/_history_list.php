...
<div class="wt_iew_history_page">
	<h2 class="wp-heading-inline"><?php _e('Import/Export history', 'wt-import-export-for-woo');?></h2>

	<div style="margin-bottom:25px;">
		<?php _e('Lists the runs and the status corresponding to every import/export with options to re-run, view detailed log or delete entry.', 'wt-import-export-for-woo');?>
	</div>
	...
	<div class="wt_iew_history_settings">
		<form action="admin.php">
			...
			{
			?>
				<div class="wt_iew_history_settings_hd"><?php _e('Filter', 'wt-import-export-for-woo'); ?></div>
						...
						?>
							<div class="wt_iew_history_settings_form_group">
								...
								<select name="wt_iew_history[filter_by][<?php echo $filter_by_key;?>]" class="wt_iew_select">
									<option value=""><?php _e('All', 'wt-import-export-for-woo'); ?></option>
									<?php
			...
			<div class="wt_iew_history_settings_form_group_box">
				<div class="wt_iew_history_settings_form_group">
					<label><?php _e('Sort by', 'wt-import-export-for-woo'); ?></label>
				...
				<div class="wt_iew_history_settings_form_group">
					<label><?php _e('Max record/page', 'wt-import-export-for-woo'); ?></label>
			...
			<div class="wt_iew_history_settings_form_group_box">
				...
				?>
				<button class="button button-primary" type="submit" style="float:left;"><?php _e('Apply', 'wt-import-export-for-woo'); ?></button>
			</div>
		</form>
	</div>
	...
	<div class="wt_iew_bulk_action_box">
		...
		<button class="button button-primary wt_iew_bulk_action_btn" type="button" style="float:left;"><?php _e('Apply', 'wt-import-export-for-woo'); ?></button>
	</div>
	<?php
	...
	if(isset($history_list) && is_array($history_list) && count($history_list)>0)
	{
		...
		<thead>
			<tr>
				...
				<th width="50"><?php _e("Id", 'wt-import-export-for-woo'); ?></th>
				<th><?php _e("Action type", 'wt-import-export-for-woo'); ?></th>
				<th><?php _e("Post type", 'wt-import-export-for-woo'); ?></th>
				<th><?php _e("Started at", 'wt-import-export-for-woo'); ?></th>
				<th>
				...
				<th>
					<?php _e("Actions"); ?>
					<span class="dashicons dashicons-editor-help wt-iew-tips" 
						data-wt-iew-tip=" <span class='wt_iew_tooltip_span'><?php _e('Re-run will take the user to the respective screen depending on the corresponding action type and the user can initiate the process accordingly.', 'wt-import-export-for-woo');?></span>"></span>
				</th>
			</tr>
		</thead>
		<tbody>
		...
		foreach($history_list as $key =>$history_item)
		{
			...
			<tr>
				...
				<td><?php echo __( ucfirst($history_item['template_type']) , 'wt-import-export-for-woo' ); ?></td>
				<td><?php echo __( ucfirst($history_item['item_type']), 'wt-import-export-for-woo' ); ?></td>
				...
				<td>
					...
					if($action_type=='import' && Wt_Import_Export_For_Woo_Admin::module_exists($action_type))
					{
						...
						{
						?>
							| <a class="wt_iew_view_log_btn" data-history-id="<?php echo $history_item['id'];?>"><?php _e('Details', 'wt-import-export-for-woo');?></a>
						<?php
						}
					}
					if($action_type=='export' && Wt_Import_Export_For_Woo_Admin::module_exists($action_type))
					{
						...
						?>
                                                        | <a class="wt_iew_export_download_btn" target="_blank" href="<?php echo $export_download_url;?>"><?php _e('Download', 'wt-import-export-for-woo');?></a>
						<?php
					}                                        
					?>
				</td>
	...
	{
		?>
		<h4 class="wt_iew_history_no_records"><?php _e("No records found.", 'wt-import-export-for-woo'); ?></h4>
		<?php
	}
	?>
</div>